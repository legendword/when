    ...
    <q-header bordered class="bg-white text-primary top-bar">
      <q-toolbar>
        <q-btn
          ...
          flat
          round
          :icon="menu ? 'chevron_left' : 'chevron_right'"
          @click="menu = !menu"
        />
        ...
        <div class="categories-container">
          ...
          <q-list dense padding class="menu-list" v-if="categories.length > 0">
            <q-item
              ...
              :class="
                'text-weight-medium' +
                (categoryFilter != null && categoryFilter == category.id
                  ? ' shadow-5'
                  : '')
              "
            ...
            >
              ...
              <q-popup-proxy
                :value="editCategory.id == category.id"
                @input="editCategoryPopupChange"
              >
                <q-card class="q-pa-sm">
                  <q-input
                    class="category-input-wrapper"
          ...
          <div
            v-else-if="categoriesLoaded"
            class="text-center text-grey-8 text-caption q-my-md"
          >
...
export default {
  ...
  methods: {
    categoryClick(id) {
      this.$store.commit(
        "layout/filterCategory",
        this.categoryFilter === id ? null : id,
      );
    },
    ...
    createCategory() {
      if (
        ...
        this.newCategory.color.length != 7
      )
        return;
      categoryUtil
        .add(this.newCategory.name, this.newCategory.color)
        .then(() => {
    ...
    submitEditCategory() {
      categoryUtil
        .edit(this.editCategory.id, { ...this.editCategory })
        .then(() => {
    ...
    handlePageVisibilityChange() {
      this.$store.commit(
        "layout/pageVisibilityChange",
        !document[this.pageHiddenProp],
      );
    },
  },
  created() {
