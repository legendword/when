    ...
    <div class="full-width">
      ...
      <div class="q-my-md">
        <q-chip
          ...
          :key="category.id"
          clickable
          :outline="universalValue.category != category.id"
          :style="
            universalValue.category != category.id
          ...
          "
          @click="
            universalValue.category =
              universalValue.category == category.id ? null : category.id
          "
        >
          {{ category.name }}
        </q-chip>
        ...
        <q-tab-panel name="event" class="ne-tab-panel">
          <div class="ne-row">
            <div class="ne-icon">
              <q-icon
                name="schedule"
                :color="inFocus.date ? 'primary' : 'default'"
                size="20px"
              />
            </div>
            <div class="ne-input ne-date-wrapper">
              <date-input
                ...
                @focus="dateFocus('dateFrom')"
                @blur="dateBlur('dateFrom')"
                ref="event-dateFrom"
              />
              <time-input
                v-if="!value.fullDay"
                v-model="value.timeFrom"
                @focus="dateFocus('timeFrom')"
                @blur="dateBlur('timeFrom')"
                @remove="removeTime"
                :removeBtn="true"
                ref="event-timeFrom"
              />
              ...
              <template v-if="value.dateTo != null">
                ...
                <date-input
                  ...
                  @focus="dateFocus('dateTo')"
                  @blur="dateBlur('dateTo')"
                  ref="event-daeTo"
                />
                <time-input
                  v-if="!value.fullDay"
                  v-model="value.timeTo"
                  @focus="dateFocus('timeTo')"
                  @blur="dateBlur('timeTo')"
                  ref="event-timeTo"
                />
              </template>
          ...
          <div class="ne-row">
            <div class="ne-icon pd">
              <q-btn
                :color="inFocus.notes ? 'primary' : 'default'"
                round
                flat
                dense
                size="12px"
            ...
            <div class="ne-input">
              <q-input
                ...
                label="Notes"
                autogrow
                @focus="inFocus.notes = true"
                @blur="inFocus.notes = false"
                ref="notes"
              />
            </div>
          </div>
          ...
          <div class="ne-row">
            <div class="ne-icon">
              <q-icon
                ...
                :color="
                  inFocus.date &&
                  (currentActive == 'dateFrom' || currentActive == 'timeFrom')
                    ? 'primary'
                    : 'default'
                "
                size="20px"
            ...
            <div class="ne-input ne-date-wrapper">
              ...
              <date-input
                v-model="deadlineValue.dateFrom"
                @focus="dateFocus('dateFrom')"
                @blur="dateBlur('dateFrom')"
                ref="deadline-dateFrom"
              />
              <time-input
                v-model="deadlineValue.timeFrom"
                @focus="dateFocus('timeFrom')"
                @blur="dateBlur('timeFrom')"
                ref="deadline-timeFrom"
              />
            </div>
          </div>
          <div class="ne-row">
            <div class="ne-icon">
              <q-icon
                ...
                :color="
                  inFocus.date &&
                  (currentActive == 'dateTo' || currentActive == 'timeTo')
                    ? 'primary'
                    : 'default'
                "
                size="20px"
            ...
            <div class="ne-input ne-date-wrapper">
              ...
              <date-input
                v-model="deadlineValue.dateTo"
                @focus="dateFocus('dateTo')"
                @blur="dateBlur('dateTo')"
                ref="deadline-dateTo"
              />
              <time-input
                v-model="deadlineValue.timeTo"
                @focus="dateFocus('timeTo')"
                @blur="dateBlur('timeTo')"
                ref="deadline-timeTo"
              />
    ...
    submit() {
      ...
      if (this.type == "deadline") {
        ...
        let ddl = {
          ...this.deadlineValue,
          startDate:
            this.deadlineValue.dateFrom + " " + this.deadlineValue.timeFrom,
        ...
        if (
          ...
          this.value.dateTo &&
          this.value.dateFrom &&
          this.value.dateTo < this.value.dateFrom
        ) {
          this.$q.notify({
            type: "negative",
            message: "Invalid date range.",
