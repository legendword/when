<template>
  <div
    :class="'calendar-event text-subtitle2' + (active ? ' shadow-5' : '')"
    :title="event.title + (!event.fullDay ? ' ' + event.timeFrom : '')"
    :style="categoryHelper.calendarItemStyle(event)"
  >
    <div
      :class="
        'event-title ellipsis' +
        (event.isTodo ? ' todo' : '') +
        (event.isTodo && event.done ? ' strikethrough' : '')
      "
    >
      {{ event.title }}
    </div>
    <div v-if="!event.fullDay" class="event-time text-grey-7">
      {{ event.timeFrom }}
    </div>

    <q-popup-proxy
      @before-show="$emit('setActive')"
      @before-hide="$emit('removeActive')"
    >
      <q-card style="min-width: 400px">
        <q-bar
          class="q-pl-md"
          :style="categoryHelper.itemBackgroundStyle(event)"
        >
          <div class="text-body2 text-weight-medium">
            {{ event.isTodo ? "Todo" : "Event" }}
          </div>
          <q-space />
        ...
        <q-card-section>
          <div class="text-h6 calendar-popup-title">
            ...
            <div>
              <q-chip
                v-if="event.category != null"
                :style="categoryHelper.itemBackgroundStyle(event)"
                >{{ categoryHelper.categoryName[event.category] }}</q-chip
              >
            </div>
          </div>
          <template v-if="event.dateTo && event.dateTo != event.dateFrom">
            <!-- multi-day events -->
            <div class="text-body2">
              <span style="display: inline-block; width: 42px">From </span>
              <span>{{ humanWeekDate(event.dateFrom) }}</span>
              <span v-if="!event.fullDay" class="text-grey-7">
                {{ event.timeFrom }}</span
              >
            </div>
            <div class="text-body2">
          ...
          <div class="text-body2" v-else>
            ...
            <span v-if="!event.fullDay" class="text-grey-7">
              {{ event.timeFrom }}</span
            >
            <span v-if="!event.fullDay && event.dateTo" class="text-grey-7">
              - {{ event.timeTo }}</span
            >
          </div>
          <div class="q-mt-md event-notes" v-if="event.notes.length > 0">
            {{ event.notes }}
          </div>
        </q-card-section>
        <q-separator />
        <q-card-actions align="right" v-if="event.type == 'todo'">
          <q-btn
            ...
            no-caps
            class="text-grey-7 text-subtitle2"
