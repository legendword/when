<template>
  ...
  >
    <q-linear-progress
      ...
      size="40px"
      :value="value"
      :color="completed ? 'positive' : behind ? 'warning' : 'primary'"
      track-color="grey-3"
    ...
    <q-linear-progress
      ...
      class="time-progress progress-overlay"
    />
    <svg
      height="45"
      width="10"
      class="pointer pointer-time"
      :style="{ left: timePointer - 5 + 'px' }"
    >
      <path d="M 0 0 L 5 5 L 10 0 L 0 0 M 4 4 L 4 45 L 6 45 L 6 4 Z" />
    </svg>
    <div
      class="pointer-text pointer-text-time column justify-center text-center"
      :style="{ left: timePointer - 35 + 'px' }"
    >
      <div>Time</div>
    ...
    <svg
      ...
      :class="
        ...
        (completed ? ' completed' : behind ? ' behind' : '')
      "
      :style="{ left: progressPointer - 5 + 'px' }"
    >
      <path
        d="M 0 45 L 5 40 L 10 45 L 0 45 M 4 0 L 4 41 L 5 40 L 6 41 L 6 0 Z"
      />
    </svg>
    <div
      :class="
        'pointer-text pointer-text-progress column justify-center text-center' +
        (completed ? ' completed' : behind ? ' behind' : '')
      "
      :style="{ left: progressPointer - 35 + 'px' }"
    >
...
export default {
  ...
  computed: {
    behind() {
      //whether progress is behind time
