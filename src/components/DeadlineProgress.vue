<template>
    <div class="deadline-progress" v-touch-pan.horizontal.prevent.mouse="handlePan" @click="handleClick" ref="container">
        <q-linear-progress rounded size="40px" :value="value" :color="behind?'warning':'primary'" track-color="grey-3" class="progress-progress" />
        ...
        <svg height="45" width="10" :class="'pointer pointer-progress' + (behind?' behind':'')" :style="{left: (progressPointer - 5) + 'px'}">
            <path d="M 0 45 L 5 40 L 10 45 L 0 45 M 4 0 L 4 41 L 5 40 L 6 41 L 6 0 Z" />
        </svg>
        <div :class="'pointer-text pointer-text-progress column justify-center text-center' + (behind?' behind':'')" :style="{left: (progressPointer - 35) + 'px'}">
