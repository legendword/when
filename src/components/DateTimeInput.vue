<template>
  <div>
    <q-input
      ...
      :label-color="active ? 'primary' : 'default'"
      @click="toggleActive()"
    >
    ...
    <q-slide-transition>
      <div v-show="active" class="q-pl-xl">
        <div class="q-my-sm row items-center">
          <div
            v-for="(info, name) in dateComponentInfo"
            :key="name"
          ...
          >
            ...
            <q-input
              outlined
              dense
              :value="dateComponents[name]"
              :maxlength="info.len"
              :style="{ width: name == 'year' ? '4rem' : '2.5rem' }"
              input-class="text-center"
              @focus="selectAll(info.id)"
  ...
  computed: {
    ...
    dateComponents() {
      ...
      if (!dateStr) {
        ...
        //position also stored in const dateComponentPos
        return {
  ...
  methods: {
    ...
    updateValue(name, val) {
      ...
      if (info.isValid(val)) {
        ...
        let newVal =
          this.value.substr(0, info.pos) +
          newStr +
          this.value.substr(info.pos + info.len);
