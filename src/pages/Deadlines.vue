        ...
        <q-card>
          <q-card-section>
            <div class="row justify-between">
              <div class="text-h5">
                <span :style="categoryHelper.itemTextStyle(ddl)">{{
                  ddl.title
                }}</span>
              ...
              <div>
                <q-btn
                  v-if="ddl.completeDate == null"
                  class="q-mr-sm"
                  color="primary"
                  label="Complete"
                  @click="completeDeadline(ind)"
                />
                <q-btn
                  v-else
                  class="q-mr-sm"
            ...
            <deadline-progress
              :value="ddl.progress"
              @input="progressChange(ind, $event)"
              :time-value="timeProgress[ddl.id]"
              :completed="ddl.completeDate != null"
              class="q-mx-md"
            />
            <div class="q-pt-sm text-body2 row justify-between">
              <div class="q-pr-sm">
                {{ isBefore(ddl.startDate) ? "Started" : "Starts" }}
                {{ calendarTime(ddl.startDate) }}
              </div>
              <div v-if="ddl.completeDate != null" class="q-px-sm">
                Completed {{ calendarTime(ddl.completeDate) }}
              </div>
...
export default {
  ...
  methods: {
    ...
    editDeadline(ind, isHistory = false) {
      this.editddl = isHistory
        ? this.historyDeadlines[ind]
        : this.deadlines[ind];
